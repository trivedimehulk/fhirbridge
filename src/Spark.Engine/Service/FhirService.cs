...
namespace Spark.Engine.Service
{
        ...
        //A future step might be to split that part into a different service (maybe StorageService?)
    {
        ...
        public FhirResponse Create(IKey key, Resource resource)
        {
            ...
            //From here it will go to real time Availity with RMS wrapper - MT
            //Entry result = Store(Entry.POST(key, resource));

            EligibilityResponse _r= new EligibilityResponse();
            _r.Id = "E2500";
            _r.Identifier = new List<Identifier>();
            _r.Identifier.Add(new Identifier { System= "http://www.BenefitsInc.com/fhir/eligibilityresponse", Value="12345" });
            _r.Request = new ResourceReference();
            _r.Request.Reference = "http://localhost:49911/fhir/EligibilityRequest/1";
            _r.Outcome = RemittanceOutcome.Complete;
            _r.Disposition = "Policy is currently in-force.";

            return Respond.WithResource(_r);
        }
